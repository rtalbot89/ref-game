...
function updateEntities(dt) {
    ...
    for (var i = 0; i < right_students.length; i++) {
...
function checkCollisions(dt) {
    ...
    for (var i = 0; i < students.length; i++) {
        var pos = students[i].pos;
        var size = students[i].sprite.size;
    ...
    for (var i = 0; i < right_students.length; i++) {
        var pos = right_students[i].pos;
        var size = right_students[i].sprite.size;
...
function renderSlots() {
    slots = [];
    var correctSlot = 1;
    var slotWidth = canvas.width / slotIds.length;
    
    for (var i = 0; i < slotIds.length; i++) {
        var width = canvas.width / slotIds.length;
        var height = slotHeight;
        var color = "white";
        var x = i === 0 ? 0 : (canvas.width / slotIds.length) * i;
        var y = 0;
        ...
        ctx.stroke();
        var slot = new component({ pos: [x, y], size: [width, height], slotId: slotIds[i] });
        slots.push(slot);
    }
}
